(function(){
  ...
  function parseMarkdown(md){
    ...
    while(i<lines.length){
      ...
      if(/^###\s+/.test(line)){
        ...
        i++;
        while(i<lines.length && /^(tags|标签|difficulty|难度|knowledge|知识点)\s*:\s*/i.test(trim(lines[i]))){
          var kv = trim(lines[i]).split(':'); var k=trim(kv[0]).toLowerCase(); var v=trim(kv.slice(1).join(':'));
          if(k==='tags' || k==='标签'){ q.tags = parseTags(v); }
          else if(k==='difficulty' || k==='难度'){ q.difficulty = v; }
          else if(k==='knowledge' || k==='知识点'){ q.knowledge = v; }
          i++;
        }
        while(i<lines.length && trim(lines[i])==='') i++;
        if(i<lines.length && /^题目\s*[:：]/.test(trim(lines[i]))){ i++; }
        var body=[]; while(i<lines.length && !/^选项\s*[:：]/.test(trim(lines[i])) && !/^答案\s*[:：]/.test(trim(lines[i])) && !/^解析\s*[:：]/.test(trim(lines[i])) && !/^###\s+/.test(trim(lines[i]))){ var tl=trim(lines[i]); if(isOptionLine(tl)) break; body.push(lines[i]); i++; }
        q.body = cleanupBody(body.join('\n'));
        if(i<lines.length && /^选项\s*[:：]/.test(trim(lines[i]))){ i++; var opts=[]; while(i<lines.length && !/^答案\s*[:：]/.test(trim(lines[i])) && !/^解析\s*[:：]/.test(trim(lines[i])) && !/^###\s+/.test(trim(lines[i]))){ var t = trim(lines[i]); if(isOptionLine(t)){ opts.push(t); } else if(t!=='') { opts.push(t); } i++; } q.options = opts; }
        else if(i<lines.length && isOptionLine(trim(lines[i]))){ var opts=[]; while(i<lines.length && !/^答案\s*[:：]/.test(trim(lines[i])) && !/^解析\s*[:：]/.test(trim(lines[i])) && !/^###\s+/.test(trim(lines[i]))){ var t = trim(lines[i]); if(isOptionLine(t)){ opts.push(t); } else if(t!==''){ opts.push(t); } i++; } q.options = opts; }
        if(i<lines.length && /^答案\s*[:：]/.test(trim(lines[i]))){ q.answer = trim(lines[i]).replace(/^答案\s*[:：]/,''); i++; }
        if(i<lines.length && /^解析\s*[:：]/.test(trim(lines[i]))){ i++; var sol=[]; while(i<lines.length && !/^###\s+/.test(trim(lines[i]))){ sol.push(lines[i]); i++; } q.solution = sol.join('\n'); }
        res.push(q);
        continue;
      }
    ...
    return { title: paperTitle, questions: res };
  }
  function renderPaper(p){
    var titleEl = document.getElementById('paper-title'); if(titleEl) titleEl.textContent = p.title || '试卷';
    var container = document.getElementById('questions'); var empty = document.getElementById('empty');
  ...
  function attachEvents(p){
    var container = document.getElementById('questions'); var search = document.getElementById('search'); var toggleAll = document.getElementById('toggleAnswers');
    if(search){
      search.addEventListener('input', function(){ var q = trim(search.value).toLowerCase(); var nodes = container.querySelectorAll('.question');
        nodes.forEach(function(n){ var text = n.textContent.toLowerCase(); n.style.display = q && text.indexOf(q)===-1 ? 'none' : ''; });
      });
    }
    if(container){
  ...
  function tryFetch(urls){
    ...
    function next(resolve,reject){
      if(u.length===0){ reject(new Error('not found')); return; }
      var url=u.shift();
      fetch(url).then(function(r){
        var isFile = (typeof location!=='undefined' && location.protocol==='file:');
        if(!isFile && !r.ok) throw new Error('fail');
        return r.text();
      }).then(function(txt){
        if(!txt || !String(txt).trim()) throw new Error('empty');
        resolve({ url:url, text:txt });
      }).catch(function(){ next(resolve,reject); });
    }
